...
const Drones = () => {
  ...
  const addDrones = () => {
    for (let i = 0; i < 5; i++) {
      axios
        .post("http://127.0.0.1:8000/drone/", { name: `DRONE_AA_0${i}` })
        .then((response) => {
          console.log(response);
        });
    }
  };
  const deleteAllDrones = (id: number) => {
    axios
     ...
     <div className="grid grid-cols-8 gap-4">
        ...
        <div className="col-end-10 col-span-6">
          <div className="grid grid-cols-4 gap-6 justify-center">
            ...
            <button
              type="button"
              onClick={() => deleteAllDrones(data[0]?.id)}
              ...
              <div className="flex ">
                ...
                {data && Array.isArray(data) ? (
                    ...
                    className="w-40 mr-3 bg-white dark:bg-gray-700 p-1 rounded-lg shadow-md ring-2 ring-gray-200 dark:ring-gray-700">
                      <div className="flex justify-between">
                        ...
                        <div className={`h-4 w-4  rounded-full ${drone.status.toLowerCase() === "on" ? "bg-green-500" : "bg-red-500"}`}></div>
                      </div>
                      <div className="flex justify-center items-center ">
                        <DroneIcon width={70} height={70} strokeWidth="2" color={`${drone.status.toLowerCase() === "on" ? "green":"red"}`} />
                      </div>
                      <div className="flex justify-between">
                        <div className="text-xs text-gray-500 dark:text-gray-300">
                          {drone.name.toLowerCase().replace("drone_", "").toUpperCase()}
                        </div>
                        <div className="text-xs text-gray-500 dark:text-gray-300">
                          {drone.status_order}
                        </div>
                ...
                <div className="w-11/12 h-screen ml-2 p-2 dark:bg-gray-800 bg-white rounded-lg shadow-md ring-2 ring-gray-200 dark:ring-gray-700 bg-gradient-to-br from-gray-800 to-gray-600  ">
                    {/* <div className="w-6/12 text-lg object-center break-words ">
                      ...
                      {droneInfo ? (<div className="flex col-auto gap-2">
